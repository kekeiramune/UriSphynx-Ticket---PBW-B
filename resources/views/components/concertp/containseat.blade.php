<div class="grid grid-cols-2 gap-4 max-w-[900px] mx-auto mt-8 mb-8">
    <div class="w-[300px] h-[300px] bg-white border border-gray-300 rounded-[24px] px-8 py-[120px] text-center">
        <h1 class="text-3xl mb-5 font-bold">VIP</h1>
        <p class="text-xl text-[#9795B5]">PRICE 1</p>
    </div>
    <div class="w-[300px] h-[300px] bg-white border border-gray-300 rounded-[24px] px-8 py-[120px] text-center">
        <h1 class="text-3xl mb-5 font-bold">CAT 1</h1>
        <p class="text-xl text-[#9795B5]">PRICE 2</p>
    </div>
    <div class="w-[300px] h-[300px] bg-white border border-gray-300 rounded-[24px] px-8 py-[120px] text-center">
        <h1 class="text-3xl mb-5 font-bold">CAT 2</h1>
        <p class="text-xl text-[#9795B5]">PRICE 3</p>
    </div>
    <div class="w-[300px] h-[300px] bg-white border border-gray-300 rounded-[24px] px-8 py-[120px] text-center">
        <h1 class="text-3xl mb-5 font-bold">CAT 3</h1>
        <p class="text-xl text-[#9795B5]">PRICE 4</p>
    </div>
</div>