...
<div x-data="{ expanded: false }" class="w-full flex flex-col items-center">
    <div class="mt-6 flex gap-8 justify-center flex-wrap px-4 md:px-20 w-full">
        @forelse($activeTickets as $ticket)
            <div x-show="expanded || {{ $loop->index }} < 3"
                class="shadow-xl shadow-black/20 w-full md:w-[300px] h-auto p-6 md:p-0 md:h-[400px] bg-white rounded-[24px] flex flex-col items-center justify-center mb-6 transition-all duration-300 transform"
                ...
                @endif
                <h1 class="font-bold text-[#2C2C2C] text-xl mt-5 text-center px-4">{{ $ticket->concert->concert_name }}</h1>
    ...
    @if($activeTickets->count() > 3)
        <button @click="expanded = !expanded" 
            class="text-[#707FDD] font-semibold hover:text-[#5f6bc9] transition-colors flex items-center gap-2 mb-8">
